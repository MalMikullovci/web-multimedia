...
const Banner = ({ movies = [] }) => {
  ...
  useEffect(() => {
    setDisplayedMovies(movies.slice(0, 5)); // Initially load the first 5 movies
  }, [movies]);
  ...
  }, [lastMovieElementRef, fetchMoreMovies]);
        ...
        <TransitionGroup className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-8 gap-6">
          {displayedMovies.map((movie, index) => {
            if (displayedMovies.length === index + 1) {
              return (
                <CSSTransition key={movie.id} timeout={300} classNames="fade">
                  <Link to={`/movie/${movie.id}`} ref={lastMovieElementRef} className="relative group text-white rounded-lg overflow-hidden shadow-md transition duration-300 transform hover:scale-105 border-none">
                    <div className="relative pb-[150%] overflow-hidden bg-gray-900 rounded-lg border-none">
                      <img
                        className="absolute h-full w-full object-cover transition duration-300 group-hover:opacity-50 rounded-lg border-none"
